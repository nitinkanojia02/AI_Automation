...
- Do not leave successful authentication tests with only URL/location checks when the approved manual expectedResult requires authenticated state, authenticated identity, authenticated-only controls, or another business-visible destination outcome
- Do not resolve a missing stronger validation by pretending a weak page/form/URL check is equivalent; preserve the strongest approved observable evidence available and keep the missing validation capability visible as a refinement gap instead of inventing confidence
