...
- Do not leave successful authentication or guest-state tests with only URL/location checks when the approved manual expectedResult requires authenticated state, guest state, authenticated identity, authenticated-only controls, guest-only controls, or another business-visible destination outcome and approved reusable page/resource evidence exists for those stronger assertions
...
- Do not overclaim page state or business state in test wording or assertions beyond reviewed observable evidence already grounded in approved resources
