...
- If the approved manual wording expresses a concrete user action such as clicking a named control, submitting a form, returning through browser/application navigation, or opening the target page through an upstream journey, ensure the suite preserves that action explicitly through approved imported keywords instead of replacing it with a generic placeholder flow
