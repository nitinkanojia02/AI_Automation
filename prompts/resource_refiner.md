...
- Apply that canonical-variable normalization not only to single-control action keywords but also to page-state, page-load, visible-state, enabled-state, and multi-control verification keywords whenever the retained controls are grounded in approved page metadata
...
- Preserve reviewed keyword names exactly when they already express a correct approved page capability; do not compress, fuse words, partially rewrite approved keyword names, or split one approved page capability into weaker near-duplicate names during refinement
- If higher-level reviewed keywords validate a page state, page load, or a set of grounded controls, keep those higher-level capabilities intact and canonicalize the variable references inside the implementation instead of degrading them into lower-level aliases or thinner wrappers
