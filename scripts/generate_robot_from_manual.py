...
def build_prompt(manual_data: dict, resource_context: List[Dict]) -> str:
    ...
    return (
        ...
        "- For negative authentication scenarios, do not rely solely on Verify Login Page Loaded. Include at least one stronger observable assertion such as an error message check, validation message check, no-navigation check, protected-area-not-visible check, or another resource-backed visible rejection check.\n"
...
def build_manual_review_prompt(manual_data: dict) -> str:
    ...
    return (
        ...
        "- preserve story-mandated scenarios such as explicit navigation controls, approved credential usage, page-entry flows, and post-transition authenticated-state validations before optional exploratory edge cases\n"
        "- if the manual artifact contains generic 'valid credentials' wording but the source workflow provided approved credential values, refine the positive login scenarios so the approved values remain visible in title, steps, or expectedResult\n"
        "- if successful-login acceptance criteria mention authenticated-state evidence such as logged-in user name or authenticated-only features, ensure those become explicit manual test coverage rather than remaining implied inside a generic success case\n\n"
        "Rules:\n"
        "- return only JSON\n"
