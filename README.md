...
## Non-Negotiable Generation Principle

The framework must not rely on workflow-specific Python mappings, hardcoded keyword substitutions, hardcoded locator substitutions, or page-specific repair tables to generate downstream automation.

Instead, each stage must derive and persist reusable knowledge artifacts from:
- the approved workflow input and business story
- approved upstream resource files
- approved variables, locators, keywords, and validations already generated by prior workflows
- inferred workflow reuse context that identifies which existing resources are authoritative for the downstream journey
...
The current repo is a working MVP and is currently demonstrated with curated `login` and `home` workflow examples, with the deepest coverage still centered on login.
...
Some additional folders such as `pom_pages/`, `manual_tests/`, and `tests/` are generated by the pipeline at runtime. This repository intentionally also keeps a small curated set of approved sample artifacts checked in so a clean checkout still demonstrates the end-to-end output shape.
...
The repository now includes sample checked-in workflow definitions for the curated `login` and `home` examples so the documented directory exists in a clean checkout.
